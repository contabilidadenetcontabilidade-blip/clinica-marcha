...
app.put('/api/athletes/:id/captain', (req, res) => {
  ...
  const { is_captain } = req.body;

  if (typeof is_captain === 'undefined') {
    return res.status(400).json({ error: "Campo is_captain obrigatório." });
  }

  db.serialize(() => {
    db.run("BEGIN TRANSACTION");
    db.run(
      "UPDATE athletes SET is_captain = ? WHERE id = ?",
      [is_captain ? 1 : 0, athleteId],
      function (err) {
        if (err) {
          db.run("ROLLBACK");
        ...
        // Se nomeando capitão, registra bônus no log
        if (is_captain) {
          db.get(
            "SELECT house_id, patient_id FROM athletes WHERE id = ?",
            [athleteId],
            (err, ath) => {
              if (ath) {
                db.run(
                  "INSERT INTO house_points_log (house_id, student_id, points_awarded, description) VALUES (?, ?, ?, ?)",
                  [
                    ath.house_id,
                    ath.patient_id,
                    0,
                    `Capitão nomeado. Bônus mensal de +5 meinhas será aplicado automaticamente.`,
                  ]
                );
              }
              db.run("COMMIT");
              res.json({
                success: true,
                message: is_captain
                  ? "Capitão nomeado com sucesso!"
                  : "Capitão removido com sucesso.",
              });
            }
          );
        } else {
          ...
          res.json({
            success: true,
            message: "Capitão removido com sucesso.",
          });
        }
      }
    );
...
app.delete('/api/patients/:id', (req, res) => {
  ...
  db.serialize(() => {
    db.run("BEGIN TRANSACTION");
    db.run("DELETE FROM student_cards WHERE student_id = ?", [patientId]);
    db.run("DELETE FROM scores WHERE athlete_id = ?", [patientId]);
    db.run("DELETE FROM house_points_log WHERE student_id = ?", [patientId]);
    db.run("DELETE FROM campo_batalha_pendentes WHERE patient_id = ?", [patientId]);
    db.run("DELETE FROM card_queue WHERE attacker_id = ?", [patientId]);
    db.run("DELETE FROM active_effects WHERE beneficiary_id = ?", [patientId]);
    ...
    db.run("DELETE FROM patients WHERE id = ?", [patientId], function(err) {
      ...
      if (this.changes === 0) { db.run("ROLLBACK"); return res.status(404).json({ error: "Paciente não encontrado." }); }
      db.run("COMMIT");
      res.json({ success: true, message: "Aluno excluído completamente do sistema." });
    });
  });
});
          ...
          db.get("SELECT a.house_id FROM athletes a WHERE a.patient_id = ?", [patient_id], (err, ath) => {
            if (!err && ath && ath.house_id) {
              if (status === 'confirmado' || status === 'realizado') {
                ...
                db.get(
                  "SELECT id FROM active_effects WHERE house_id = ? AND effect_type = 'ZICA' AND expires_at > ? LIMIT 1",
                  [ath.house_id, nowIso],
                  (err, zicaEffect) => {
                    const faltaValue = zicaEffect ? -4 : -2;
                    const faltaDesc = zicaEffect
                      ? `Falta sem justif. (ZICA ativa, dobro): ${sanitizedTitle}`
                      : `Falta sem justif.: ${sanitizedTitle}`;
                    db.run(
                      "INSERT INTO house_points_log (house_id, student_id, points_awarded, description) VALUES (?, ?, ?, ?)",
                      [ath.house_id, patient_id, faltaValue, faltaDesc]
                    );
                  }
                );
              }
...
app.get('/api/ranking', (req, res) => {
  ...
  const query = `
    ...
    LEFT JOIN scores sc ON a.id = sc.athlete_id
    LEFT JOIN scoring_rules sr ON sc.rule_id = sr.id
    WHERE h.active = 1
    GROUP BY h.id
    ORDER BY total_points DESC
...
app.get('/api/active-effects', (req, res) => {
  const { house_id, type } = req.query;
  if (!house_id) return res.status(400).json({ error: "house_id obrigatório." });

  const nowIso = new Date().toISOString();
  let query = "SELECT * FROM active_effects WHERE house_id = ? AND expires_at > ?";
  let params = [house_id, nowIso];
        ...
        db.get("SELECT id, beneficiary_id, house_id FROM active_effects WHERE effect_type = 'CORINGA_REDIRECTION' AND source_captain_id = ? AND expires_at > ? LIMIT 1", [athleteId, nowIso], (err, coringaActive) => {
          ...
          db.get(
            ...
            db.get("SELECT COUNT(*) as count FROM athletes WHERE house_id = ? AND active = 1", [athlete.house_id], (err, hRow) => {
              ...
              db.get(`
              SELECT 
                (SELECT COALESCE(SUM(sr.value), 0) FROM scores s JOIN athletes a ON s.athlete_id = a.patient_id JOIN scoring_rules sr ON s.rule_id = sr.id WHERE a.house_id = ? AND s.created_at LIKE ?) +
...
app.get('/api/student-portal/:id', (req, res) => {
  ...
  db.get("SELECT * FROM patients WHERE id = ?", [patientId], (err, patient) => {
    ...
    db.get("SELECT * FROM athletes WHERE patient_id = ?", [patientId], (err, athlete) => {
        ...
        db.all(historyQuery, [patientId, patientId], (err, scores) => {
          ...
          res.json({
            ...
            athlete: { 
              ...
              is_captain: athlete ? (athlete.is_captain === 1) : false
            },
            scores: scores || []
          });
        });
...
app.post('/api/cards/grant', (req, res) => {
  const { patient_id, card_name } = req.body;
      ...
      data.forEach((row, idx) => {
        ...
        db.get("SELECT a.id as athlete_id, a.house_id, a.patient_id FROM athletes a JOIN patients p ON a.patient_id = p.id WHERE p.name = ?", [nome], (err, athlete) => {
          if (err || !athlete) {
            ...
            // Mapeamento Motivo -> rule_id em scoring_rules (agora as regras fixas estão em 'scoring_rules')
            db.get("SELECT id, value FROM scoring_rules WHERE name = ?", [motivo], (err, ruleRow) => {
              ...
              db.run("INSERT INTO scores (athlete_id, rule_id) VALUES (?, ?)", [athlete.patient_id, rule_id], function (err) {
                if (err) { errors.push(`Erro inserindo score para ${nome} (Regra ${rule_id}): ${err.message}`); checkDone(); return; }
                db.run("INSERT INTO house_points_log (house_id, student_id, points_awarded, description) VALUES (?, ?, ?, ?)",
                  [athlete.house_id, athlete.patient_id, points_to_award, motivo], (err) => {
...
// --- CONSOLIDAÇÃO MENSAL (Bônus Capitães) ---
function runMonthlyBonus() {
  const now = new Date();
  // Só executa no dia 1 de cada mês
  if (now.getDate() !== 1) return;

  const monthKey = now.toISOString().substring(0, 7); // ex: 2026-03

  console.log(`[MENSAL] Processando bônus de capitães para ${monthKey}...`);

  // Busca todos os capitães ativos
  db.all(
    `SELECT a.id, a.patient_id, a.house_id, p.name 
     FROM athletes a 
     JOIN patients p ON a.patient_id = p.id 
     WHERE a.is_captain = 1`,
    [],
    (err, captains) => {
      if (err) { console.error('[MENSAL] Erro ao buscar capitães:', err.message); return; }
      if (!captains.length) { console.log('[MENSAL] Nenhum capitão encontrado.'); return; }

      captains.forEach(captain => {
        // Verifica se o bônus já foi dado neste mês
        db.get(
          "SELECT id FROM house_points_log WHERE student_id = ? AND description LIKE ? AND created_at LIKE ?",
          [captain.patient_id, '%Bônus Capitão%', monthKey + '%'],
          (err, existing) => {
            if (existing) return; // Já recebeu o bônus este mês

            db.run(
              "INSERT INTO house_points_log (house_id, student_id, points_awarded, description) VALUES (?, ?, ?, ?)",
              [captain.house_id, captain.patient_id, 5, `Bônus Capitão: +5 meinhas mensais (${monthKey})`],
              (err) => {
                if (err) console.error(`[MENSAL] Erro ao registrar bônus para ${captain.name}:`, err.message);
                else console.log(`[MENSAL] Bônus +5 registrado para capitão: ${captain.name}`);
              }
            );
          }
        );
...
// Verifica a cada hora se é dia 1 para rodar o bônus mensal
setInterval(runMonthlyBonus, 60 * 60 * 1000);
// Também roda ao iniciar o servidor (caso o servidor reinicie no dia 1)
runMonthlyBonus();

// --- CONSOLIDAÇÃO SEMANAL (Cartas por Mérito) ---
function runWeeklyConsolidation() {
  const now = new Date();
  // Só executa aos sábados às 23:59
  if (now.getDay() !== 6) return;
  if (now.getHours() !== 23 || now.getMinutes() !== 59) return;

  const weekKey = now.toISOString().substring(0, 10); // ex: 2026-03-15
  console.log(`[SEMANAL] Processando consolidação semanal para ${weekKey}...`);
  ...
  // 1) RARA: Casa com maior total de meinhas na semana
  db.all(
    `SELECT house_id, COALESCE(SUM(points_awarded), 0) as total 
    ...
    [mondayIso],
    (err, topHouses) => {
      if (!err && topHouses.length > 0) {
        const topHouse = topHouses[0];
        // Distribui carta Rara para todos os atletas da casa campeã da semana
        db.all(
          "SELECT patient_id FROM athletes WHERE house_id = ? AND active = 1",
          [topHouse.house_id],
          (err, athletes) => {
            if (!err && athletes.length) {
              db.get("SELECT id FROM cards WHERE rarity = 'Rara' ORDER BY RANDOM() LIMIT 1", [], (err, card) => {
                if (card) {
                  athletes.forEach(ath => {
                    const crypto = require('crypto');
                    const hash = crypto.randomBytes(8).toString('hex').toUpperCase();
                    db.run("INSERT INTO student_cards (student_id, card_id, hash) VALUES (?, ?, ?)",
                      [ath.patient_id, card.id, hash]);
                  });
                  db.run("INSERT INTO house_points_log (house_id, student_id, points_awarded, description) VALUES (?, NULL, 0, ?)",
                    [topHouse.house_id, `Semanal: Casa campeã da semana! Carta Rara distribuída para todos (${weekKey})`]);
                  console.log(`[SEMANAL] Carta Rara distribuída para casa ${topHouse.house_id}`);
                }
              });
            }
          }
  ...
  // 2) ÉPICA: Maior pontuador individual da semana
  db.get(
    `SELECT s.athlete_id, COALESCE(SUM(sr.value), 0) as total
     FROM scores s
     JOIN scoring_rules sr ON s.rule_id = sr.id
     JOIN athletes a ON s.athlete_id = a.patient_id
     WHERE s.created_at >= ?
    ...
    (err, topPlayer) => {
      if (!err && topPlayer) {
        db.get("SELECT id FROM cards WHERE rarity = 'Épica' ORDER BY RANDOM() LIMIT 1", [], (err, card) => {
          if (card) {
            const crypto = require('crypto');
            const hash = crypto.randomBytes(8).toString('hex').toUpperCase();
            db.run("INSERT INTO student_cards (student_id, card_id, hash) VALUES (?, ?, ?)",
              [topPlayer.athlete_id, card.id, hash]);
            console.log(`[SEMANAL] Carta Épica distribuída para maior pontuador: ${topPlayer.athlete_id}`);
          }
        });
      }
    }
  ...
  // 3) LENDÁRIA: Casa em último lugar no mês recebe Golpe de Estado
  const currentMonthLike = now.toISOString().substring(0, 7) + '%';
  db.all(
    `SELECT house_id, COALESCE(SUM(points_awarded), 0) as total
     FROM house_points_log
    ...
    [currentMonthLike],
    (err, lastPlace) => {
      if (!err && lastPlace.length > 0) {
        const last = lastPlace[0];
        db.all(
          "SELECT patient_id FROM athletes WHERE house_id = ? AND is_captain = 1",
          [last.house_id],
          (err, captains) => {
            if (!err && captains.length) {
              db.get("SELECT id FROM cards WHERE name = 'Golpe de Estado' LIMIT 1", [], (err, card) => {
                if (card) {
                  captains.forEach(cap => {
                    const crypto = require('crypto');
                    const hash = crypto.randomBytes(8).toString('hex').toUpperCase();
                    db.run("INSERT INTO student_cards (student_id, card_id, hash) VALUES (?, ?, ?)",
                      [cap.patient_id, card.id, hash]);
                  });
                  console.log(`[SEMANAL] Carta Golpe de Estado distribuída para capitães da casa em último: ${last.house_id}`);
                }
              });
            }
          }
...
// --- REVELAÇÃO CORINGA STEALTH ---
function runCoringaRevelation() {
  const nowIso = new Date().toISOString();

  db.all(
    `SELECT ae.id, ae.source_captain_id, ae.beneficiary_id, 
            ae.house_id as beneficiary_house, ae.stolen_points_accumulator,
            p1.name as cap_name, a1.house_id as cap_house, 
            p2.name as ben_name
    ...
    [nowIso],
    (err, expiredCoringas) => {
      if (err) { console.error('[CORINGA] Erro na revelação:', err.message); return; }
      if (!expiredCoringas || !expiredCoringas.length) return;
      ...
      expiredCoringas.forEach(coringa => {
        ...
        db.serialize(() => {
          db.run("BEGIN TRANSACTION");
          // Débito da casa vítima
          db.run(
            "INSERT INTO house_points_log (house_id, student_id, points_awarded, description) VALUES (?, ?, ?, ?)",
            [cap_house, coringa.source_captain_id, -stolen_points_accumulator,
             `REVELAÇÃO CORINGA: -${stolen_points_accumulator} pontos desviados por ${ben_name} durante 7 dias.`]
          );
          // Crédito da casa beneficiária
          db.run(
            "INSERT INTO house_points_log (house_id, student_id, points_awarded, description) VALUES (?, ?, ?, ?)",
            [beneficiary_house, coringa.beneficiary_id, stolen_points_accumulator,
             `REVELAÇÃO CORINGA: +${stolen_points_accumulator} pontos coletados de ${cap_name}!`]
          );
          // Remove o efeito
          db.run("DELETE FROM active_effects WHERE id = ?", [id]);
          db.run("COMMIT", (err) => {
            if (err) console.error('[CORINGA] Erro no commit:', err.message);
            else console.log(`[CORINGA] Revelação processada com sucesso.`);
          });
        });
      });
    }
...
// Verifica a cada hora se há Coringas expirados para revelar
setInterval(runCoringaRevelation, 60 * 60 * 1000);
// Roda ao iniciar também
runCoringaRevelation();
console.log('[CORINGA] Revelação automática ativa (verificação a cada hora)');
